...
def load_model_checkpoint(ckpt_path: str, device="cpu"):
    ...
    model = MLPRegressor(
        ...
        output_dim=int(ckpt.get("output_dim", 3)),
    ...
    scaler = {
        "mean": np.array(ckpt["scaler"]["mean"], dtype=np.float64).reshape(1, -1),
        "std": np.array(ckpt["scaler"]["std"], dtype=np.float64).reshape(1, -1),
    }
    return ckpt, model, scaler
...
def main():
    ...
    y_true = build_target(data).astype(np.float64)

    ckpt, model, scaler = load_model_checkpoint(args.model, device=args.device)
    use_log_features = bool(ckpt["args"].get("use_log_features", False))

    X = build_features(data, use_log_features=use_log_features)
    Xs = apply_scaler(X, scaler)
    pred_neural = predict_model(model, Xs, device=args.device).astype(np.float64)
