...
from grid_search_multidim_hybrid_correction import (
    load_npz, build_inputs_and_baseline, load_model_checkpoint,
    refine_batch, residual_metrics, vector_metrics, save_csv, apply_scaler
)
...
def main():
    ...
    seq_x, glob_x, y_true, z0 = build_inputs_and_baseline(data, use_log_features=use_log_features)
    seq_shape = seq_x.shape
    seq_x = apply_scaler(seq_x.reshape(-1, seq_x.shape[-1]), seq_scaler).reshape(seq_shape)
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(seq_x), bs):
            ...
            pred, _ = model(s, g, z, qt)
            preds.append(pred.cpu().numpy())
    pred_direct = np.concatenate(preds, axis=0)
    heur_pred = z0.copy()

    rows = []
    hd = vector_metrics(heur_pred, y_true)
    hd.update(residual_metrics(heur_pred, data))
    ...
    hr.update(residual_metrics(href, data))
    ...
    nd = vector_metrics(pred_direct, y_true)
    nd.update(residual_metrics(pred_direct, data))
    nd['name'] = 'neural_direct'
    rows.append(nd)

    nref, nit, nconv = refine_batch(pred_direct.astype(np.float64), data, tol=args.tol, max_iter=args.max_newton_iter)
    nr = vector_metrics(nref, y_true.astype(np.float64))
    nr.update(residual_metrics(nref, data))
    nr['name'] = 'neural_plus_newton'
    nr['newton_iter_mean'] = float(np.mean(nit))
    nr['newton_iter_median'] = float(np.median(nit))
    nr['newton_iter_p90'] = float(np.percentile(nit, 90))
    nr['newton_converged_ratio'] = float(np.mean(nconv))
    rows.append(nr)
    ...
    for i in range(len(y_true)):
        per_rows.append({
            ...
            'heur_Q1': float(heur_pred[i, 0]),
            'heur_x1': float(heur_pred[i, 1]),
            'heur_x2': float(heur_pred[i, 2]),
            'pred_Q1': float(pred_direct[i, 0]),
            'pred_x1': float(pred_direct[i, 1]),
            'pred_x2': float(pred_direct[i, 2]),
            'ref_Q1': float(nref[i, 0]),
            'ref_x1': float(nref[i, 1]),
            'ref_x2': float(nref[i, 2]),
            'iter': int(nit[i]),
            'converged': bool(nconv[i]),
        })
