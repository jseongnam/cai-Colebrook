...
"""
evaluate_multidim_colebrook_models.py

목적
----
train_multidim_colebrook_models.py 로 학습한
MLP / LSTM / GRU / Transformer 체크포인트를 공통 방식으로 평가한다.
...
def build_model_from_ckpt_args(ckpt_args: Dict, seq_dim: int, seq_len: int, glob_dim: int) -> nn.Module:
    ...
    if model_name == "mlp":
        ...
        return TransformerModel(
            ...
            nhead=int(ckpt_args.get("nhead", 3)),
            num_layers=int(ckpt_args.get("num_layers", 2)),
            dropout=float(ckpt_args.get("dropout", 0.1)),
            out_dim=3,
            ff_dim=int(ckpt_args.get("ff_dim", 256)),
            head_hidden=int(ckpt_args.get("head_hidden", 128)),
        )
    else:
