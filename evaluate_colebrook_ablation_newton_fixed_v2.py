...
# -*- coding: utf-8 -*-
"""
evaluate_colebrook_ablation_newton_fixed_v2.py

fix:
- CSV 저장 시 첫 row 기준 fieldnames를 쓰지 않고,
  모든 row의 key 합집합을 fieldnames로 사용
- checkpoint dropout 구조 복원 유지
"""
...
def newton_refine_vectorized(
    ...
    for k in range(1, max_iter + 1):
        ...
        small = np.abs(dfx) < 1e-14
        dfx[small] = np.where(dfx[small] >= 0, 1e-14, -1e-14)
...
def main():
    ...
    zero_newton, zero_iters, zero_conv = newton_refine_vectorized(
    ...
    heur_newton, heur_iters, heur_conv = newton_refine_vectorized(
    ...
    for model_name, model_path in model_pairs:
        ...
        pred_newton, pred_iters, pred_conv = newton_refine_vectorized(
