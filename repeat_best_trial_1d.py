...
class NPZRootDataset1D(Dataset):
    def __init__(
        ...
        self.glob_x = (
            ...
            if feats else np.zeros((coeffs.shape[0], 0), dtype=np.float32)
        )
...
def build_model(model_name: str, seq_dim: int, seq_len: int, glob_dim: int, hp: Dict):
    ...
    if model_name == "transformer":
        return TransformerRegressor(
            ...
            hp["d_model"], hp["nhead"], hp["num_layers"], hp["dropout"],
            hp["ff_dim"], hp["head_hidden"], hp["head_layers"], hp["use_cls_token"]
        )
...
def mean_std_summary(rows: List[Dict], numeric_keys: List[str]) -> Dict:
    out = {}
    for k in numeric_keys:
        vals = [float(r[k]) for r in rows if k in r and r[k] is not None and np.isfinite(float(r[k]))]
        if vals:
...
def load_best_hp(best_result_json: str) -> Dict:
    with open(best_result_json, "r", encoding="utf-8") as f:
        best = json.load(f)

    hp = {
        "model": best["model"],
        "use_log_ab": bool(best["hp_use_log_ab"]),
        "optimizer": best["hp_optimizer"],
        "criterion": best["hp_criterion"],
        "dropout": float(best["hp_dropout"]),
        "lr": float(best["hp_lr"]),
        "weight_decay": float(best["hp_weight_decay"]),
        "hidden_dims": json.loads(best["hp_hidden_dims"]) if isinstance(best["hp_hidden_dims"], str) else best["hp_hidden_dims"],
        "hidden_size": int(best["hp_hidden_size"]),
        "num_layers": int(best["hp_num_layers"]),
        "head_hidden": int(best["hp_head_hidden"]),
        "head_layers": int(best["hp_head_layers"]),
        "d_model": int(best["hp_d_model"]),
        "nhead": int(best["hp_nhead"]),
        "ff_dim": int(best["hp_ff_dim"]),
        "use_cls_token": bool(best["hp_use_cls_token"]),
    }
    return hp
...
def main():
    ...
    parser.add_argument("--best_result_json", type=str, required=True)
    ...
    hp = load_best_hp(args.best_result_json)
    ...
    for repeat_idx in range(1, args.n_repeats + 1):
        ...
        run_name = f"repeat_{repeat_idx:03d}"
        print(f"\n========== {run_name} ==========")
        print(json.dumps({"seed": seed, **hp}, ensure_ascii=False))
        ...
        if best_state is None:
            raise RuntimeError(f"No best_state found for {run_name}")
        ...
        pred_refined, iters, conv = newton_refine_batch(
            pred_direct, a, b, tol=args.tol, max_iter=args.max_newton_iter
        )
        ...
        row = {
            ...
            "direct_max_abs_error": direct_row["max_abs_error"],
            ...
            "plus_newton_max_abs_error": plus_row["max_abs_error"],
        ...
        if best_metric is None or row["plus_newton_rmse"] < best_metric:
            best_metric = row["plus_newton_rmse"]
            best_run = dict(row)
            torch.save(
                {
                ...
                out_dir / "best_model_among_repeats.pt"
            )

    save_csv(out_dir / "repeat_results.csv", all_rows)
    ...
    summary = {
        ...
        "best_result_json": args.best_result_json,
        "best_run": best_run,
        "aggregate": mean_std_summary(all_rows, numeric_keys),
        "hp": hp,
    }
    ...
    print("\n================ REPEAT SUMMARY ================")
    print(json.dumps(summary["best_run"], ensure_ascii=False, indent=2))
    print("\n[DONE]")
    print(out_dir / "repeat_results.csv")
    print(out_dir / "repeat_summary.json")
    print(out_dir / "best_model_among_repeats.pt")
