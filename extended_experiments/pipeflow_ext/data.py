...
def dataset_fingerprint(data: Mapping[str, np.ndarray]) -> str:
    """Return a deterministic content ID independent of NPZ compression metadata."""
    digest = hashlib.sha256()
    for key in sorted(data):
        value = np.ascontiguousarray(np.asarray(data[key]))
        digest.update(key.encode("utf-8")); digest.update(b"\0")
        digest.update(value.dtype.str.encode("ascii")); digest.update(b"\0")
        digest.update(json.dumps(value.shape).encode("ascii")); digest.update(b"\0")
        if value.dtype.kind in {"U", "S"}:
            digest.update(str(value.tolist()).encode("utf-8"))
        else:
...
def dataset_record(path: str | Path, root: str | Path | None = None) -> dict:
    path = Path(path)
