...
def summarize(files: list[Path], keys: list[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    defaults = {
        ...
        "dataset_id": "unknown",
        "train_dataset_id": "unknown",
        "val_dataset_id": "unknown",
    }
    if "inference_batch_size" in keys:
        defaults["inference_batch_size"] = 0
...
def main():
    ...
    raw, summary = summarize(
        eval_files,
        ["suite_id", "train_dataset_id", "val_dataset_id", "dataset_id", "branches",
         "regime", "input_mode", "experiment_variant", "target_mode",
