...
최종 논문용으로 모든 실험의 데이터를 완전히 통일하려면
`CANONICAL_SUITE_README_KO.md`와 `run_canonical_suite.sh`를 사용한다. 이 경로는
각 NPZ의 content fingerprint를 동결하고 checkpoint와 결과 CSV에도 같은 suite ID를 기록한다.
